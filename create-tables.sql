...
CREATE TABLE `CATALOGUENUMBERS` (
  ...
  `NUMBERSORT` varchar(35) DEFAULT '0',
  PRIMARY KEY (`ID`),
  ...
  KEY `NUMBERSORT` (`NUMBERSORT`) USING BTREE,
...
CREATE TABLE `OWNERSHIP` (
  ...
  CONSTRAINT `FK_OWNERSHIP_ALBUM_ID` FOREIGN KEY (`ALBUM_ID`) REFERENCES `ALBUMS` (`ID`) ON DELETE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `SEQUENCE_GEN` (`ID_NAME`, `ID_VAL`) VALUES ('OWNERSHIP_ID', 0);
INSERT INTO `SEQUENCE_GEN` (`ID_NAME`, `ID_VAL`) VALUES ('CATEGORY_ID', 0);
INSERT INTO `SEQUENCE_GEN` (`ID_NAME`, `ID_VAL`) VALUES ('CATNUM_ID', 0);
INSERT INTO `SEQUENCE_GEN` (`ID_NAME`, `ID_VAL`) VALUES ('COLLECTION_ID', 0);
INSERT INTO `SEQUENCE_GEN` (`ID_NAME`, `ID_VAL`) VALUES ('STAMP_ID', 0);
INSERT INTO `SEQUENCE_GEN` (`ID_NAME`, `ID_VAL`) VALUES ('ALBUM_ID', 0);
INSERT INTO `SEQUENCE_GEN` (`ID_NAME`, `ID_VAL`) VALUES ('COUNTRY_ID', 0);
INSERT INTO `SEQUENCE_GEN` (`ID_NAME`, `ID_VAL`) VALUES ('VERSION_ID', 0);
INSERT INTO `SEQUENCE_GEN` (`ID_NAME`, `ID_VAL`) VALUES ('CATALOGUE_ID', 0);
INSERT INTO `SEQUENCE_GEN` (`ID_NAME`, `ID_VAL`) VALUES ('PREF_ID', 0);
INSERT INTO `SEQUENCE_GEN` (`ID_NAME`, `ID_VAL`) VALUES ('SELLER_ID', 0);
...
END$$
CREATE TRIGGER `ALBUM_BDEL` BEFORE DELETE ON `ALBUMS` FOR EACH ROW
BEGIN
	DELETE FROM `OWNERSHIP` WHERE ALBUM_ID=OLD.ID;
END$$
CREATE TRIGGER `OWNERSHIP_AUPD` AFTER UPDATE ON `OWNERSHIP` FOR EACH ROW
BEGIN
    IF @trigger_updated != 1 THEN
          SET @modifyStamp=NOW();
          UPDATE `STAMPS` AS s SET s.MODIFYSTAMP=@modifyStamp WHERE s.ID=OLD.STAMP_ID;
          SET @trigger_updated = 1;
    END IF;
END$$
CREATE TRIGGER `OWNERSHIP_ADEL` AFTER DELETE ON `OWNERSHIP` FOR EACH ROW
BEGIN
    SET @modifyStamp=NOW();
    UPDATE `STAMPS` AS s SET s.MODIFYSTAMP=@modifyStamp WHERE s.ID=OLD.STAMP_ID;
END$$
...
END$$
CREATE TRIGGER `COUNTRY_BDEL` BEFORE DELETE ON `COUNTRIES` FOR EACH ROW
BEGIN
	DELETE FROM `ALBUMS_COUNTRIES` WHERE COUNTRY_ID=OLD.ID;
END$$
DELIMITER ;